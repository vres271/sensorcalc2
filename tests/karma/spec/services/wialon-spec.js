describe('Wialon Service', function() {
    var Wialon, httpBackend;
    beforeEach(module('Main'));
    beforeEach(inject(function(_Wialon_, _$httpBackend_) {
        Wialon = _Wialon_;
				...
				function(method, url, data) {
					...
					if(data.sid) {
						if(data.sid === saved_sid) {
							return [200,{"tm":1500039089,"events":[]}];		
						}
	...
	var expectWialonclear = function() {
		expect(Wialon.auth).toEqual(false);
		expect(Wialon.user).toEqual(null);
		expect(Wialon.sid).toEqual(undefined);
    	expect(sessionStorage.getItem('sid')).toEqual(null);
	}

	var expectWialonInited = function(sid) {
	  	expect(Wialon.auth).toEqual(true); 
	  	expect(Wialon.sid).toEqual(sid); 
	  	expect(Wialon.user.nm).toBe('glomosru');
    	expect(sessionStorage.getItem('sid')).toEqual(sid);
	}

    it('can get an instance of my service', inject(function(Wialon) {
        expect(Wialon).toBeDefined();
    }));

    it('it is clear', function() {
		expect(Wialon.testmode).toEqual(true);
		expectWialonclear();
    });
    ...
    it('setSID()', function() {
    	...
    	Wialon.setSID(saved_sid);
    	expect(Wialon.sid).toEqual(saved_sid);
    	expect(sessionStorage.getItem('sid')).toEqual(saved_sid);
    ...
    it('start() with right token', function() {
    	var response;
		Wialon.start(function(data) {response = data;},{token: token});

		httpBackend.flush();

	  	expect(typeof response).toBe('object');

		expectWialonInited(saved_sid);
	  	
    });

    it('start() with wrong token', function() {
    	var response;
		Wialon.start(function(data) {response = data;},{token: wrong_token});
    ...
    it('start() when SID exists - right', function() {
    	var response;
		Wialon.start(function(data) {response = data;},{token: token});
		httpBackend.flush();
		
		Wialon.start(function(data) {response = data;});
    ...
    it('start() when SID exists - wrong - relogin', function() {
    	var response;
		Wialon.start(function(data) {response = data;},{token: token});
		httpBackend.flush();
    	
    	Wialon.setSID(wrong_sid);
		Wialon.start(function(data) {response = data;});
    ...
    it('duplicate() - right sid', function() {
    	var response;
		Wialon.duplicate(function(data) {response = data;},saved_sid);
		httpBackend.flush();

	  	expect(typeof response).toBe('object');
	  	expectWialonInited(dubl_sid);
	  	
    });

    it('duplicate() - wrong sid', function() {
    	var response;
		Wialon.duplicate(function(data) {response = data;},wrong_sid);
		httpBackend.flush();
    ...
    it('stop()', function() {
		...
		Wialon.stop(function(data) {callback_called = true;});
